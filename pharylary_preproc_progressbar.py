...
if os.path.exists(output_csv):
    print(f"Loading VoiceSauce data — output already exists at {output_csv}")
    voicesauce_data = pd.read_csv(output_csv, encoding='utf-8')
...
for filename in os.listdir(textgrid_folder):
    if filename.endswith('.TextGrid'):  # Check if the file is a TextGrid file
        ...
        all_relevant_data.to_csv(out_csv, index=False)
...
for filename in os.listdir(textgrid_folder):
    if filename.endswith('.TextGrid'):  # Check if the file is a TextGrid file
        ...
        out_csv = os.path.join(output_dir, f"{tg_basename}_relevant.csv")

        # Skip if we've already produced this per-file CSV
        if os.path.exists(out_csv):
            print(f"Skipping {filename} — already have {out_csv}")
            continue
        ...
        # slice voicesauce data to just the textgrid
        voicesauce_data_grid = voicesauce_data[(voicesauce_data['Filename'].str.contains(tg_basename, na=False))]

        # Collect all chunks for THIS TextGrid, then save a single CSV
        tg_chunks = pd.DataFrame()
        ...
        for phrasenum_interval in phrasenum_tier:
            if phrasenum_interval.mark is not str(''):
                phrasenum_value = phrasenum_interval.mark
                # print(tg_basename)
                # print("Processing %s..."%phrasenum_value)

                # Subset the VoiceSauce output for the current phrasenum
                # print("Subsetting...")
                subset_voicesauce = voicesauce_data_grid[
                                    (voicesauce_data_grid['Label'] == int(phrasenum_value))
                                ]

                # Process each tier
                for tier_name, tier in tiers.items():
                    # print("Processing %s..."%tier)
                    for interval in tier:
                        if interval.mark != '':
                            start_time = interval.minTime # these originally have a multiplier for 1000, unsure why, but check later pipeline
                            end_time = interval.maxTime
                            ...
                            # tg_chunks.append(relevant_data)
                            tg_chunks = pd.concat([tg_chunks, relevant_data], ignore_index=True)
        ...
        # Save THIS TextGrid's output as a single CSV
        # if tg_chunks:
            # tg_df = pd.concat(tg_chunks, ignore_index=True)
        tg_df.to_csv(out_csv, index=False)
        print(f"Saved {len(tg_df)} rows -> {out_csv}")
        # else:
        #     # write an empty file (optional), or just log
        #     print(f"No relevant rows for {filename}; nothing saved.")
